...
Текущая версия: **0.207.79**; immutable fallback — **0.207.79rc0**. Owner-only
...
conversation и admission-order anchors сообщений. Исторические разговоры
остаются честным `backfill_pending`; writer и productive search lane в этом
релизе не активируются. Prompts,
...
Приёмка 0.207.79 должна показать в `/api/health` его точную версию,
