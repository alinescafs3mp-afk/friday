...
Текущая версия: **0.208.58** — неопубликованный release candidate.
При обновлении с 0.208.57 она же служит совместимым со schema 50 immutable
fallback; прежняя копия 0.208.56 сохраняется. Выпуск требует полной приёмки.
...
атомарный archive/expiry retirement. Schema 45 добавляет immutable body-free
binding к точному принятому ingress request; canonical schema-44 graph
...
ревизии chunk-контракта. Schema 48 атомарно переводит только
passage-sidecar на document-only v3: непрогрессирующие spans отсеиваются,
а exact stored-child locator перепроверяется при replay. Schema 49 добавляет
reader-first body-free sidecar разговоров: одна immutable projection на
conversation и admission-order anchors сообщений. Schema 50 активирует
bounded restart-safe writer и partial conversation-lexical lane внутри
`archive_search`; полная `MESSAGE_HISTORY` остаётся fail-soft fallback и
единственным правом доказывать отсутствие. Prompts,
...
Приёмка 0.208.58 должна показать в `/api/health` его точную версию,
