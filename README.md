...
jericho backfill-person-entities [--user U] [--batch N] [--limit N] [--apply]
...
- **`backfill-person-entities`** — проводит правило ФИО (отчество как объявление «вот человек») по архиву, загруженному ДО появления этого правила. Нужна потому, что правило действует только на новые поступления: в графе владельца было 110 сущностей и ни одного человека, при том что почти все его вопросы — про людей. Замерено на живом архиве: 20 644 упоминания, 4349 различных ФИО, 71% имён встречается больше чем в одном документе, перестановочных двойников («Фамилия Имя Отчество» против «Имя Отчество Фамилия») — 4 имени из 4349. По умолчанию только считает; запись включает `--apply`. Связи, по которым человек уже высказался, обходятся: проход не отменяет решений владельца.
