...
Текущая версия: **0.166.0**. Это release-candidate / 1.0-ready сборка: умеренная классификация, активный граф знаний, управляемая многошаговая работа, миссии с управляемой автономией, замкнутый feedback loop и полноценные эксплуатационные контуры без скрытой автоматической записи.
...
- Поиск материалов, загруженных выбранным человеком в общий архив, применяет точное `uploaded_by` на исходном Raw Object до FTS/LIKE `LIMIT`; чужие и неатрибутированные документы не могут ни попасть в ответ, ни вытеснить нужный. Эта shared-дорога явно помечена `scoped_lexical`: полный uploader-scoped dense/reranker/graph остаётся отдельным улучшением качества, а не скрытым обещанием.
