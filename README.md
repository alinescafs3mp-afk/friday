...
добавляет выключенный fixed WorkGraph ровно для сравнения текущего
файла с текущим вебом: два body-free read-шага, primary synthesis и
CAS/restart state, один review-admitted web recovery, exact full/terminal
publication receipts и атомарный archive/expiry retirement. Исполняющего
worker/scheduler, web/model execution и runtime route здесь нет; явная отмена
закрывает graph отдельным code-owned non-completion receipt. Schema 43
