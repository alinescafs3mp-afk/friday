...
Текущая версия: **0.9.0**. Это release-candidate / 1.0-ready сборка: умеренная классификация, активный граф знаний, управляемая многошаговая работа, миссии с управляемой автономией, замкнутый feedback loop и полноценные эксплуатационные контуры без скрытой автоматической записи.
...
5. Версия 0.8.0 использует ту же SQLite schema 8, что и 0.7.0: авторитетные знания, граф, Inbox и разговоры не переписываются. При открытии могут идемпотентно достраиваться отсутствующие производные projections вроде usage state. Более новая неизвестная схема отклоняется без изменений.
