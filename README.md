...
Текущая версия: **0.153.0**. Это release-candidate / 1.0-ready сборка: умеренная классификация, активный граф знаний, управляемая многошаговая работа, миссии с управляемой автономией, замкнутый feedback loop и полноценные эксплуатационные контуры без скрытой автоматической записи.
...
- Ранжирование, Agent Runtime и `memory_search` используют один и тот же ограниченный `graph_context`, без повторного обхода графа. `as_of` означает valid-time и применяется на каждом шаге; исторический обход не подмешивает сегодняшние implicit `co_occurs_in`.
