...
## G9 (#56). Разметить очередь конфликтов: очевидное vs требует внимания

Третья очередь владельца — 200 «почти дубликатов», порог near-duplicate 0.95. Замерил
(`~/.jericho/eval/conflict_queue_is_it_real.py`, критерии объявлены до запуска):

| | доля |
|---|---:|
| настоящий дубликат (Jaccard ≥0.95, длины ±5%) | **68.0%** |
| БЛАНК — отличие в данных, не в оформлении | **15.5%** |
| непохожи вовсе — ложное срабатывание | 14.0% |
| похожи, отличие в оформлении | 2.5% |

Медиана различий у настоящих дубликатов — **одно слово**. Порог для этого корпуса
годный (доля бланков 15.5% при объявленном пределе 30%), но **массово подтверждать
очередь нельзя**: треть пар — либо две разные записи про разных людей, либо вообще
не похожи.

Здесь другой урон, чем у слияний (там неотменяемая порча), но тоже неприятный:
`conflict_decide(keep_a/keep_b)` помечает вторую сторону устаревшей. На бланке это
значит объявить устаревшим настоящий, отдельный документ.

**Что сделать.** `_conflict_list` (`execution_kernel/__init__.py:940`) уже отдаёт `a` и
`b` компактно — добавь в каждый элемент подсказку по тем же критериям, что в пробе:
- Jaccard по содержательным словам (стемы, как в `eval_bootstrap.content_tokens` —
  сравнение должно бить по тем же признакам, что видит матчер, иначе подсказка сама
  окажется неточной);
- отношение длин;
- доля различающихся слов, начинающихся с большой буквы или содержащих цифру.

Три исхода, как в пробе: `likely_duplicate` / `likely_different_records` /
`uncertain`. Это ПОДСКАЗКА, не автоматическое решение — `conflict_decide` остаётся
только за человеком, порог 0.95 не трогать без нового замера.

`/conflicts` в Telegram уже есть (твоя работа сегодня) — покажи метку рядом с парой,
чтобы владелец видел, где решение быстрое, а где нужно вчитаться.

Мутируемый тест: подсунуть пару-бланк (общий текст + разные ФИО) и пару-дубликат
(общий текст, одно слово различий), проверить, что метки не совпадают.
