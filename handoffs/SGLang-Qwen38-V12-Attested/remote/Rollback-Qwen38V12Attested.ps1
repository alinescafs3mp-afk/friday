...
function Assert-RestoredEndpoint([hashtable]$Headers) {
    ...
    Assert-ServerInfo ([string]$serverInfo.Content | ConvertFrom-Json)
    ...
    $status = 0
    try {
        Invoke-WebRequest -UseBasicParsing -Uri 'http://127.0.0.1:8001/_friday/v1/deployment-witness' -Headers $Headers -TimeoutSec 10 | Out-Null
        $status = 200
    }
    catch {
    ...
    if ($status -ne 404) {
        throw 'Restored stable proxy unexpectedly exposes the attested witness'
    }
}
