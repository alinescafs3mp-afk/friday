...
- Production state: immutable activation `clear`; backend and Telegram bridge
  ...
  Owner-confirmed execution of installed console programs is now live through
  an isolated universal Engineer runner: every exact argv or shell start needs
  ...
  the primary-only path. Bounded extraction, evidence-bound document MAP and
  complete-current-document advice may influence only the primary model's
  prompt and can never publish directly. The primary V12
...
Close the compiler/artifact loop, then return to the restored document-contour
WIP audit and the next complete ICP golden journey before ordered V12
refinement. The future Semantic Supervisor remains separately
owned and is not a dependency of current delivery; new policy and semantic
seams must stay compatible with its later arrival rather than creating a
competing kernel.
...
2. Reconcile `DOCUMENT_FILE_CONTOUR_WIP_AUDIT_2026-08-22.md` against current
   `main`, retain only proved useful work and remove only proved superseded
   residue.
...
- An authenticated Telegram owner turn can request any installed executable
  visible to the Friday service account. Friday proposes an exact argv and
  timeout; a separate Telegram approval is bound to that exact request before
  the process can start. Explicit shell commands use the same confirmation
  boundary.
- Jobs are durable and idempotent, with truthful progress, cancellation,
  bounded stdout/stderr evidence and restart-safe cleanup. Execution is pinned
  to held executable identities and runs under a delegated systemd cgroup plus
  bubblewrap workspace without host files, secrets, network or Docker access.
- P0 review closed the pre-GO fd collision, dispatcher bypasses, output-tail
  loss, fd-reuse hazards, cleanup ownership, trusted-PATH aliasing and Ubuntu
  AppArmor launch compatibility. The merged gate passed 52 adversarial mock
  cases, 12 integration cases, the release-transition cases, 159 secondary
  regressions and one real isolated command. Two clean wheel builds were
  byte-identical. Immutable activation completed `clear` with terminal receipt
  `ed41fb57530aa6945eb1036afedc7df7715dc28bfec2a46b8c31898c4a1e40a9`.
...
- Owner-only Engineer Mode is live for the private LAN scope with a real
  bubblewrap/no-network sandbox. Ordinary dialogue, documents, web, Obsidian
  and secondary-assist ownership remain unchanged.
- Native user-systemd no longer nests bubblewrap under the implicit user
  namespace created by `PrivateTmp`. Separate private runtime directories keep
  backend and bridge temporary files isolated without blocking the sandbox.
- Immutable installation now converges and receipts all seven unit/drop-in
  files and resumes after partial replacement. The release delta passed 390
  focused tests and the full static quality gate; its wheel reproduced
  byte-for-byte. Activation completed `clear` with receipt
  `6ed65c75c10cf2cc92cfda5b4e4d85c67f20447705be83dfd3b349e7b60d41ad`.

### Complete-current-document secondary assist and fail-open guards (`0.207.33`)

- Complete current text and Office documents now reach bounded GPT-OSS advice
  on bare upload and natural RU/EN summary, review, analysis and comparison
  requests; every failure preserves exactly one unchanged primary synthesis.
- Publication guards are scoped to matching effect/web routes or unmistakable
  current self-actions, so ordinary document, archive, local-search and
  informational prose is no longer discarded while unsupported completion
  claims remain blocked.
- The exact source passed 19,540 non-UI and 31 UI tests. Immutable activation
  completed `clear` with terminal receipt
...
- The optional laptop node remains demand-probed and removable; its absence
  ...
  Supervisor work is independent and has not been claimed as integrated.
...
4. Extend V12 only through the proven journey boundary; preserve an explicit
   seam for the future Semantic Supervisor and do not duplicate its policy role.
