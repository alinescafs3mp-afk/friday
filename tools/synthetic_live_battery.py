...
_A09_06_OWNED_FAULT = (
    ...
    r"(?:при\s+(?:сбое|сбоях|отказе|поломке)|"
    r"из[- ]за\s+(?:одной\s+)?(?:ошибки|сбоя|отказа|поломки)"
    r"(?:\s+или\s+(?:ошибки|сбоя|отказа|поломки))?)"
    r"(?:\s+(?:одного\s+)?(?:компонента|части|узла)(?:\s+системы)?|"
    r"\s+(?:сервера|диска|сети))?)"
    rf"{_A09_06_FAULT_EXPLANATION}"
    r"(?:\s+\((?:упад[её]т\s+сервер|оборв[её]тся\s+сеть|закончится\s+(?:диск|память))"
    r"(?:(?:,\s*|\s+или\s+)(?:упад[её]т\s+сервер|оборв[её]тся\s+сеть|"
    r"закончится\s+(?:диск|память))){0,3}\))?"
    r"(?:\s*[—–-]\s*например,?\s+при\s+(?:падении\s+сервиса|потере\s+связи|нехватке\s+ресурсов)"
    r"(?:(?:,\s*|\s+или\s+)(?:падении\s+сервиса|потере\s+связи|нехватке\s+ресурсов)){0,3})?"
)
_A09_06_WHOLE_SUBJECT = (
    r"(?:(?:вся\s+)?система(?:\s+в\s+целом)?|сервис|"
    r"(?:вс[её]\s+)?остальное|остальная\s+часть|"
    r"остальные\s+(?:компоненты|части|узлы|процессы|сервисы))"
)
_A09_06_OPERATION_PREDICATE = (
    r"(?:продолж(?:ит|ат|ила|или|ала|али)\s+(?:работать|функционировать)|"
    ...
    r"сможет\s+(?:продолжить\s+работу|корректно\s+восстановиться))"
)
...
_A09_06_BENIGN_OUTCOME_ASIDE = (
    ...
    r"пользовател(?:ь|и)\s+(?:получ(?:ит|ат)|увид(?:ит|ят))\s+(?:отч[её]т|сообщение|уведомление)|"
    r"пользователь\s+этого\s+не\s+заметит|отч[её]т\s+об\s+этом\s+не\s+нужен)\))?"
)
_A09_06_OWNED_CONTAINMENT = (
    rf"{_A09_06_WHOLE_SUBJECT}\s+(?:"
    rf"{_A09_06_OPERATION_PREDICATE}{_A09_06_OPERATION_ALTERNATIVE}|"
    r"не\s+(?:сломается|сломалась|умр[её]т|погибнет|погибла|упад[её]т|упала|рухнет)"
    r"(?:\s+(?:целиком|полностью))?(?:\s+или\s+не\s+потеряет\s+данные)?"
    rf"(?:\s+и\s+{_A09_06_OPERATION_PREDICATE}{_A09_06_OPERATION_ALTERNATIVE})?)"
    rf"{_A09_06_BENIGN_OUTCOME_ASIDE}"
)
...
_A09_06_NEGATED_COLLAPSE_TAIL = (
    r"(?:,\s*(?:а|и)\s+(?:сервис|система)\s+(?:полностью\s+)?не\s+"
    r"(?:рухнет|умр[её]т|упад[её]т|погибнет)(?:\s+(?:полностью|целиком))?)?"
)
_A09_06_OWNED_PURPOSE = re.compile(
    ...
    rf"(?:,?\s+(?:и|а)\s+{_A09_06_OPERATION_PREDICATE}{_A09_06_OPERATION_ALTERNATIVE})?"
    rf"{_A09_06_NEGATED_COLLAPSE_TAIL}"
    r"(?:,\s*(?:(?:а|и)\s+)?пользователи\s+(?:не\s+потеряют\s+данные"
    ...
    rf"{_A09_06_NEGATED_COLLAPSE_TAIL}\s*",
    re.IGNORECASE,
)
...
def _a09_06_task_faithful_purpose(message: str) -> bool:
    ...
    # A negated hope is explanatory commentary, not a quoted assertion that
    # may establish checking, the fault, or the surviving system.
    folded = re.sub(
        r",\s*а\s+не\s+просто\s+[«“\"](?:надеяться|верить),\s*"
        r"что\s+вс[её]\s+пройд[её]т[»”\"](?=[.!?\n]|\Z)",
        "",
        folded,
    )
