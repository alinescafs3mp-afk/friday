...
class ReasonerGrammarObject(BaseGrammarObject):
    """Defer constraints until reasoning and any structured-content header end.

    GPT-OSS Harmony emits a final-channel header after ``<|end|>``.  Applying
    the response grammar before that header is complete masks the header and
    produces malformed JSON/tool output (SGLang issue #31019).
    """
    ...
    def __init__(
        ...
        think_excluded_token_ids: list[int] | None = None,
        ...
        content_start_ids: list[int] | None = None,
    ):
        super().__init__()
        ...
        self.content_start_ids = tuple(content_start_ids) if content_start_ids is not None else None
        self._content_start_matcher = (
            TokenSequenceMatcher(self.content_start_ids) if self.content_start_ids is not None else None
        )
        ...
        self._matched_content_start_tokens = 0
        self._header_match_history: list[int] = []
    ...
    def maybe_init_reasoning(self, reasoning: bool):
        ...
        self.tokens_in_header = -1
        self._matched_content_start_tokens = 0
        self._header_match_history.clear()
        if reasoning:
    ...
    def _is_thinking(self):
        return self.tokens_in_think >= 0 and self.tokens_in_header == -1 and self.tokens_after_end == -1
    ...
    def transfer_state(self, token: int) -> None:
        if self._is_thinking():
            ...
            if self._content_start_matcher is None:
                raise RuntimeError("content-start matcher is unavailable")
            previous_match = self._matched_content_start_tokens
            self._header_match_history.append(previous_match)
            matched = self._content_start_matcher.advance(previous_match, token)
            self.tokens_in_header += 1
            if matched == len(self._content_start_matcher):
                self._matched_content_start_tokens = 0
                self.tokens_after_end = 0
            else:
                self._matched_content_start_tokens = matched
    ...
    def rollback_state(self):
        if self._is_thinking():
            if self._thinking_match_history:
                ...
                self.tokens_in_think -= previous_match + 1 - self._matched_think_end_tokens
                self._matched_think_end_tokens = previous_match
        elif self._is_generation():
            if self.tokens_after_end == 0:
                if self.tokens_in_header >= 0 and self._header_match_history:
                    self.tokens_after_end = -1
                    self.tokens_in_header -= 1
                    self._matched_content_start_tokens = self._header_match_history.pop()
                elif self._thinking_match_history:
                    self.tokens_after_end = -1
            ...
            if self.tokens_in_header == 0:
                ...
                if self._thinking_match_history:
                    self._matched_think_end_tokens = self._thinking_match_history.pop()
            elif self._header_match_history:
                self.tokens_in_header -= 1
                self._matched_content_start_tokens = self._header_match_history.pop()
    ...
    def _can_think_more(self):
        return (
            ...
            or self.tokens_in_think + self._matched_think_end_tokens < self.max_think_tokens
        )
    ...
    def copy(self):
        ...
        new_obj._matched_content_start_tokens = self._matched_content_start_tokens
        new_obj._header_match_history = list(self._header_match_history)
    ...
    def jump_and_retokenize(self, old_output_ids, new_output_ids, next_state):
        if self._is_generation() and self.grammar is not None:
            return self.grammar.jump_and_retokenize(old_output_ids, new_output_ids, next_state)
...
class ReasonerGrammarBackend(BaseGrammarBackend):
    def __init__(
        ...
        think_end_ids = tokenizer.encode(reasoning_parser.detector.think_end_token, add_special_tokens=False)
        ...
        self.content_start_ids = self._get_content_start_ids(reasoning_parser, tokenizer)
        self._enable_strict_thinking = enable_strict_thinking
        self.think_excluded_token_ids = self._get_think_excluded_token_ids(reasoning_parser, tokenizer)
        ...
        if self.enable_token_filter and not self.grammar_backend.is_support_token_filter:
            raise ValueError(
        ...
        self._token_filter_fn = self.grammar_backend.set_token_filter if self.enable_token_filter else None
    ...
    def _get_content_start_ids(
        ...
        if (
            detector.__class__.__name__ != "GptOssDetector"
            or detector.__class__.__module__ != "sglang.srt.parser.reasoning_parser"
        ):
            return None
        marker = "<|channel|>final<|message|>"
        content_start_ids = tokenizer.encode(marker, add_special_tokens=False)
        if not content_start_ids:
            raise ValueError(f"structured content start marker '{marker}' could not be encoded")
        return content_start_ids

    def _make_grammar_object(
        self, grammar: BaseGrammarObject | None, reasoning: bool
    ...
    def init_strict_reasoning_grammar(self, reasoning: bool) -> BaseGrammarObject | None:
    ...
    def _init_value_dispatch(self, key: tuple[str, str], reasoning: bool) -> BaseGrammarObject | None:
