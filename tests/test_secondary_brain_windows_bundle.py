...
def test_gateway_uses_distinct_file_secrets_tls_and_a_closed_route_set() -> None:
    ...
    assert "server_args.enable_deterministic_inference is not False" in launcher
    assert "server_args.moe_runner_backend != profile.moe_runner_backend" in launcher
    ...
    for temporary_path in ("client-body", "proxy", "fastcgi", "uwsgi", "scgi"):
        assert f"/tmp/{temporary_path};" in policy
