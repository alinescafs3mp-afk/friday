...
def runtime_for(settings, storage, monkeypatch, kernel, model):
    runtime = AgentRuntime(replace(settings, verify_answers=False, local_timezone="Europe/Moscow"), storage, llm=model, kernel=kernel)
    ...
    async def misclassified(user_id, message, conversation_id, **kwargs):
        return AgentContext(user_id=user_id, conversation_id=conversation_id, outward_verdict=("знание", None))

    monkeypatch.setattr(runtime, "_prepare_context", misclassified)
    for name in ("_prefetch_archived_source_if_asked", "_prefetch_the_web_if_asked", "_prefetch_person_activity", "_prefetch_archive_numbers", "_prefetch_the_archive_if_asked", "_prefetch_a_reminder_if_asked"):
...
CASES = [
    ...
    ("Изолируй суточный интервал 6 июня 2024 года и назови попавшее в него событие.", "2024-06-06", "23:59:59"),
    ("Нужен факт, привязанный именно к полуночи календарного дня 8 июня 2024 года.", "2024-06-08", "00:59:59"),
    ("Проверь временной индекс и сообщи событие за 19 февраля 2023 года.", "2023-02-19", "23:59:59"),
    ("Нужен факт, привязанный именно к полуночи календарного дня 29 февраля 2024 года.", "2024-02-29", "00:59:59"),
]
...
async def test_exact_timeline_crosses_full_caller_and_hostile_model(settings, storage, monkeypatch, question, day, until, entry, replay):
    ...
    if entry == "chat":
        ...
        result = await runtime._agentic_loop(context, question, actor, kernel.get_tool_definitions(actor), None)
        answer = context.structural_answer + str(result.get("content") or "")
    assert kernel.calls == [("what_happened", {"since": day + "T00:00:00", "until": day + "T" + until, "limit": 40})]
...
async def test_counterexamples_never_gain_a_timeline_call(settings, storage, monkeypatch, question):
    ...
    runtime = runtime_for(settings, storage, monkeypatch, kernel, model)
    await runtime.chat("alice", question, actor=ActorContext(user_id="alice", preset_key="owner", source="test"))
    assert kernel.calls == []


@pytest.mark.asyncio
@pytest.mark.parametrize("offered,corrupt", [(False, False), (True, True)])
async def test_absent_schema_and_unverified_payload_cannot_publish_events(settings, storage, monkeypatch, offered, corrupt):
    storage.ensure_user("alice", preset_key="owner")
    kernel, model = Kernel(offered=offered, corrupt=corrupt), HostileModel(replay=True)
    runtime = runtime_for(settings, storage, monkeypatch, kernel, model)
    result = await runtime.chat("alice", CASES[0][0], actor=ActorContext(user_id="alice", preset_key="owner", source="test"))
...
async def test_structural_calendar_read_preserves_independent_request(settings, storage, monkeypatch):
    ...
    await runtime._prefetch_the_timeline_if_asked(
        ...
        actor, kernel.get_tool_definitions(actor), [], [], [], context,
    )
