...
он защищает от сборки документа на пустом месте. Снимается он только при прямой
лексической просьбе создать файл, и содержимое при этом берётся из ОСНОВАНИЙ, а не из текста
...
def test_a_clear_file_request_survives_a_counter_question() -> None:
    """Мутация: убрать прямую просьбу — вежливая просьба снова без файла."""
    ...
    assert "_is_direct_file_request(request)" in source


def test_the_refusal_still_protects_an_unclear_request() -> None:
    """Обратная сторона: без прямой просьбы о файле отказ по встречному вопросу остаётся.
