...
@pytest.mark.parametrize(
    ("case_index", "predicate_name", "message"),
    [
        (
            4,
            "_a09_04_relation_graph",
            "Изолированная тестовая среда предотвращает взаимное влияние отдельных тестов, "
            "тем самым обеспечивает, что итог зависит только от кода, а не от состояния "
            "общей инфраструктуры.",
        ),
        (
            6,
            "_a09_06_relation_graph",
            "Проверка отказоустойчивости показывает: если компонент системы даёт сбой "
            "(например, сеть и база данных), остальные узлы продолжат работать, поэтому "
            "пользователь не потеряет данные.",
        ),
        (
            14,
            "_a09_14_relation_graph",
            "Каждый отдельный прогон создаёт новую изолированную базу, чтобы исключить "
            "влияние данных предыдущего запуска на результат.",
        ),
    ],
)
def test_a09_relation_graph_accepts_role_bound_paraphrases(
    case_index: int,
    predicate_name: str,
    message: str,
) -> None:
    predicate = getattr(battery, predicate_name)
    case = _cases("A", 9)[case_index - 1]
    record = _satisfying_record(case)
    record["response"]["message"] = message
...
@pytest.mark.parametrize(
    ...
    [
        (
            "_a09_04_relation_is_exact",
            "Изолированная тестовая среда предотвращает взаимное влияние отдельных тестов, "
            "тем самым обеспечивает, что итог зависит только от кодировщика, а не от "
            "состояния общей инфраструктуры.",
        ),
        (
            "_a09_04_relation_is_exact",
            "Изолированная тестовая среда предотвращает взаимное влияние отдельных тестов, "
            "тем самым обеспечивает, что итог зависит только от кода, а от состояния "
            "общей инфраструктуры.",
        ),
        (
            "_a09_04_relation_is_exact",
            "Изолированная тестовая среда предотвращает взаимное влияние отдельных тестов, "
            "а оркестратор обеспечивает, что итог зависит только от кода, а не от состояния "
            "общей инфраструктуры.",
        ),
        (
            "_a09_06_relation_is_exact",
            "Проверка отказоустойчивости показывает: если компонент системы даёт сбой "
            "(пользователь и отчёт), остальные узлы продолжат работать, поэтому пользователь "
            "не потеряет данные.",
        ),
        (
            "_a09_06_relation_is_exact",
            "Проверка отказоустойчивости показывает: если компонент системы не даёт сбой "
            "(например, сеть и база данных), остальные узлы продолжат работать, поэтому "
            "пользователь не потеряет данные.",
        ),
        (
            "_a09_06_relation_is_exact",
            "Проверка отказоустойчивости показывает: если компонент системы даёт сбой "
            "(например, сеть и база данных), остальные узлы перестанут работать, поэтому "
            "пользователь не потеряет данные.",
        ),
        (
            "_a09_06_relation_is_exact",
        ...
        (
            "_a09_06_relation_is_exact",
            "Проверка отказоустойчивости может показывать: если компонент системы даёт сбой "
            "(например, сеть и база данных), остальные узлы продолжат работать, поэтому "
            "пользователь не потеряет данные.",
        ),
        (
            "_a09_14_relation_is_exact",
            "Каждый отдельный прогон создаёт старую базу, чтобы исключить влияние данных "
            "предыдущего запуска на результат.",
        ),
        (
            "_a09_14_relation_is_exact",
            "Каждый отдельный прогон создаёт новую изолированную базу, чтобы не исключить "
            "влияние данных предыдущего запуска на результат.",
        ),
        (
            "_a09_14_relation_is_exact",
        ...
        (
            "_a09_14_relation_is_exact",
            "Каждый отдельный прогон создаёт новую изолированную базу, чтобы исключить "
            "влияние данных предыдущего запуска на результат. Остатки продолжают влиять "
            "на следующий тест.",
        ),
        (
            "_a09_14_relation_is_exact",
        ...
        (
            "_a09_14_relation_is_exact",
            "Каждый отдельный прогон может создавать новую изолированную базу, чтобы "
            "исключить влияние данных предыдущего запуска на результат.",
        ),
    ],
)
def test_a09_relation_graph_rejects_role_polarity_and_authority_mutations(
    predicate_name: str,
    message: str,
) -> None:
    assert getattr(battery, predicate_name)(message) is False
