...
# Independent Lab426 contrasts: the same wording must retain subject and polarity.
_LAB426_OWNED_PURPOSE_PAIRS = [
    (
        "0-fault_survival_disjunction",
        "Отказоустойчивость проверяют, чтобы убедиться, что система не умрёт целиком, если компонент "
        "сломается, и продолжит работать или корректно восстановится.",
        "Отказоустойчивость проверяют, чтобы убедиться, что система умрёт целиком, если компонент "
        "сломается, и продолжит работать или корректно восстановится.",
    ),
    (
        "1-recovery_safe_stop_prisboe_no_comma",
        "Проверять отказоустойчивость нужно, чтобы при сбое система корректно восстановилась.",
        "Проверять отказоустойчивость нужно, чтобы при сбое отчёт корректно восстановился.",
    ),
    (
        "2-recovery_safe_stop_prisboe_no_comma",
        "Отказоустойчивость проверяют, чтобы при сбое система корректно завершилась.",
        "Отказоустойчивость проверяют, чтобы при сбое система сломалась.",
    ),
    (
        "3-prisboe_quantifier_and_whole_synonym",
        "Отказоустойчивость проверяют, чтобы при сбое одного компонента остальные сервисы продолжили "
        "работать.",
        "Отказоустойчивость проверяют, чтобы при сбое одного компонента остальные отчёты продолжили "
        "работать.",
    ),
    (
        "4-user_conjunction_i_vs_a",
        "Проверка отказоустойчивости нужна, чтобы убедиться: если компонент сломается, система "
        "продолжит работать, и пользователи не столкнутся с полным крахом.",
        "Проверка отказоустойчивости нужна, чтобы убедиться: если компонент сломается, система "
        "продолжит работать, и пользователи столкнутся с полным крахом.",
    ),
    (
        "5-incidental_parenthetical_after_containment",
        "Проверка отказоустойчивости нужна, чтобы убедиться: если компонент сломается, система "
        "продолжит работать (пользователь увидит уведомление).",
        "Проверка отказоустойчивости нужна, чтобы убедиться: если часть системы сломается "
        "(пользователь, отчёт), система продолжит работать.",
    ),
    (
        "6-locally_negated_second_whole",
        "Проверка отказоустойчивости нужна, чтобы убедиться: если компонент сломается, система "
        "продолжит работать, а сервис не рухнет полностью.",
        "Проверка отказоустойчивости нужна, чтобы убедиться: если компонент сломается, система "
        "продолжит работать, а сервис рухнет полностью.",
    ),
    (
        "7-emphasis_metaphor_continuation",
        "Отказоустойчивость проверяют, чтобы убедиться, что система не «умрёт» или не потеряет "
        "данные, когда что-то пойдёт не так (упадёт сервер, оборвётся сеть, закончится диск). "
        "Простыми словами: это как проверить парашют перед прыжком — вы не надеетесь, что он "
        "сработает, а заранее тестируете, чтобы в критический момент он действительно спас.",
        "Отказоустойчивость проверяют, чтобы убедиться, что система «умрёт» или не потеряет данные, "
        "когда что-то пойдёт не так (упадёт сервер, оборвётся сеть, закончится диск). Простыми "
        "словами: это как проверить парашют перед прыжком — вы не надеетесь, что он сработает, а "
        "заранее тестируете, чтобы в критический момент он действительно спас.",
    ),
    (
        "8-emphasis_metaphor_continuation",
        "Отказоустойчивость проверяют, чтобы убедиться, что система не «умрёт» целиком из-за одной "
        "ошибки или сбоя, а продолжит работать или корректно восстановится.\n"
        "\n"
        "Простыми словами: это как страховка для вашего приложения. Вы намеренно ломаете отдельные "
        "части (сервер, базу данных, сеть) и смотрите, что будет. Если всё падает — вы знаете, где "
        "слабое место, и чините его до того, как это случится в бою.",
        "«Отказоустойчивость проверяют, чтобы убедиться, что система не «умрёт» целиком из-за одной "
        "ошибки или сбоя, а продолжит работать или корректно восстановится.»",
    ),
    (
        "9-quoted_nonassertion_idiom_vs_owned_adverse",
        "Отказоустойчивость проверяют, чтобы система продолжила работать, когда что-то сломается, а "
        "не просто «надеяться, что всё пройдёт».",
        "Отказоустойчивость проверяют, чтобы убедиться, что система не сломается, если выйдет из "
        "строя один её компонент (сервер, диск, сеть). Цитата: «она перестанет работать».",
    ),
    (
        "10-continuation_later_analogy_compatible",
        "Отказоустойчивость проверяют, чтобы система продолжила работать, когда что-то сломается. "
        "Простыми словами: это как страховка для вашего приложения.",
        "Отказоустойчивость проверяют, чтобы система продолжила работать, когда что-то сломается. На "
        "деле она перестанет работать, это как дом.",
    ),
]


@pytest.mark.parametrize("case_id,positive,negative", _LAB426_OWNED_PURPOSE_PAIRS)
def test_a09_06_owned_purpose_regression_pairs(case_id, positive, negative):
    assert _eval(5, positive) == [], case_id
    assert "content_semantic_group_missing" in _eval(5, negative), case_id
