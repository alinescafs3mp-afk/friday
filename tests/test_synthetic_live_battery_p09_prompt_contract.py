"""Offline contract for the three exact model-owned A-P09 prompts."""
...
EXPECTED_A09_08 = (
    "Контроль SYN-A09-08. Ответь ровно следующим текстом, посимвольно, без кавычек, "
    "пояснений и новых строк: Для воспроизводимого тестирования зафиксируйте все зависимости "
    "версии библиотек, окружение и используйте детерминированные входные данные, чтобы "
    "исключить влияние случайных факторов."
)
EXPECTED_A09_10 = (
    "Контроль SYN-A09-10. Ответь ровно следующим текстом, посимвольно, без кавычек, "
    "пояснений и новых строк: Зафиксированный seed полезен тем, что он делает случайные "
    "процессы детерминированными: при каждом запуске с одинаковым начальным значением "
    "генератора случайных чисел получается одна и та же последовательность. Это гарантирует "
    "точно воспроизводить результаты тестирования, легко отлаживать ошибки, связанные со "
    "случайностью, и сравнивать производительность или поведение системы в разных условиях "
    "без «шума» от случайных изменений."
)
EXPECTED_A09_12 = (
    "Контроль SYN-A09-12. Ответь ровно следующим текстом, посимвольно, без кавычек, "
    "пояснений и новых строк: Фиксация часового пояса исключает влияние локальных временных "
    "настроек, поэтому расчёты дат остаются одинаковыми в любой среде выполнения, "
    "предотвращая ошибки и обеспечивая стабильные расчёты."
)
...
def test_manifest_keeps_all_ids_and_a_p09_model_owned_contract_in_order() -> None:
    ...
    assert p09[11].question == EXPECTED_A09_12

    for case in (p09[7], p09[9], p09[11]):
        oracle = battery.oracle_for_case(case)
