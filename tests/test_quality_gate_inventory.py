...
def test_checked_in_inventory_closes_semantics_tiers_parameters_and_host_boundaries() -> None:
    ...
    assert sum(rule.scratch_mb for rule in value.rules if rule.tier != "nightly") == 14_671
    ...
    assert len(p0h) == 222 and sum(rule.node_count for rule in p0h) == 286 and sum(rule.scratch_mb for rule in p0h) == 317
