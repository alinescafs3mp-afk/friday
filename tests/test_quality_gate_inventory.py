...
def test_checked_in_inventory_closes_semantics_tiers_parameters_and_host_boundaries() -> None:
    ...
    # C1-C3 added 47 MiB to the historical 15,402 MiB; include the exact
    # C4/C5 through C11 revision/model/privacy-fixture declarations as well.
    # F.0/F.1 added four change-unit functions at 1 MiB each.
    # F.6 mixed-journey consume durability added four change-unit functions at
    # 1 MiB each.
    ...
    # Two AST cache isolation controls inherit the router module's64 MiB policy (+128).
    # Candidate091 adds three CRLF integrity regression functions at64 MiB each.
    # Harness092 declares 28 additional functions, with 13 MiB total scratch;
    # all pre-existing resource policies remain unchanged.
    ...
    # Document protected-set coverage replaces two 1 MiB functions and adds
    # 33 declarations with 62 MiB total scratch (+60 MiB net).
    # Owner Telegram declares224 MiB (160 unit +64 real bootstrap); A09 adds0.
    # Preserve every existing function budget and the exact aggregate check.
    # Telegram startup repairs add160 MiB across six explicit declarations.
    # SQLite lifetime controls add256 MiB; producer retention adds92 MiB.
    # Backup recovery coverage adds 25 functions with 3,463 MiB total scratch;
    # all prior per-function budgets remain unchanged.
    # Diagnostic collect-all adds two 1 MiB policy controls and two 8 MiB
    # real process/deadline controls; canonical budgets remain unchanged.
    # Diagnostic boundary closure adds two 1 MiB regression functions.
    # Exact scratch/socket geometry controls add three host-tool functions at 8 MiB each.
    # Five A09 semantic-oracle functions add 1 MiB each; existing budgets stay exact.
    # Two report-identity controls add2 MiB; two chat-preview regressions add16 MiB.
    # Nine A09 fault-grammar regression functions add1 MiB each; prior budgets remain exact.
    assert sum(rule.scratch_mb for rule in value.rules if rule.tier != "nightly") == 54_710
    ...
    preview = ("tests/test_the_feed_counts_files_by_their_author.py::test_feed_preview_matches_thread_tail_when_message_times_are_equal", "tests/test_the_feed_counts_files_by_their_author.py::test_feed_preview_keeps_message_time_ahead_of_insertion_order")
    assert all((rules[name].invariant_id, rules[name].tier, rules[name].execution_kind, rules[name].max_runtime_s, rules[name].scratch_mb) == ("conversation.context-reply-and-continuity", "change", "unit", 300, 8) for name in preview)
