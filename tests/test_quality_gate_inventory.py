...
def test_checked_in_inventory_closes_semantics_tiers_parameters_and_host_boundaries() -> None:
    ...
    assert sum(rule.scratch_mb for rule in value.rules if rule.tier != "nightly") == 14_742
    ...
    assert len(p0h) == 230 and sum(rule.node_count for rule in p0h) == 306 and sum(rule.scratch_mb for rule in p0h) == 388
