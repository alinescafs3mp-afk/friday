...
def test_process_lease_is_exclusive_and_recoverable(tmp_path):
    ...
    first = ProcessLease(path, protocol="friday.test.v1")
    second = ProcessLease(path, protocol="friday.test.v1")
    first.acquire()
    try:
        metadata = json.loads(path.read_text(encoding="utf-8"))
        assert metadata["protocol"] == "friday.test.v1"
...
def test_process_lease_inspection_distinguishes_active_and_stale_metadata(tmp_path):
    ...
    lease = ProcessLease(path, protocol="friday.backend.v1")
    lease.acquire()
    try:
        active = inspect_process_lease(path, protocol="friday.backend.v1")
    ...
    stale = inspect_process_lease(path, protocol="friday.backend.v1")
...
def test_process_lease_inspection_rejects_broken_symlink(tmp_path):
    ...
    result = inspect_process_lease(path, protocol="friday.backend.v1")
