...
def test_real_path_recall_and_measurement_gaps_are_closed(
    ...
    assert all(not item.gap_codes for item in conversation_run.measurements)
    assert all(
...
def test_fallback_and_foreign_saturation_channels_remain_exact(
    conversation_run: EphemeralConversationRecallRunV1,
) -> None:
    history_only = [
        item
        for item in conversation_run.measurements
        if item.projection_contour in {"backfill_pending", "source_changed", "accepted_boundary"}
    ]
    assert len(history_only) >= 4
    assert {item.match_channels for item in history_only} == {(ArchiveMatchChannel.MESSAGE_HISTORY,)}
    foreign_saturated = [
        item for item in conversation_run.measurements if item.projection_contour == "foreign_saturated"
    ]
    assert len(foreign_saturated) == 3
    assert {item.matrix_cell: item.match_channels for item in foreign_saturated} == {
        "fallback": (ArchiveMatchChannel.LEXICAL, ArchiveMatchChannel.MESSAGE_HISTORY),
        "diversity": (ArchiveMatchChannel.LEXICAL, ArchiveMatchChannel.MESSAGE_HISTORY),
        "privacy": (ArchiveMatchChannel.LEXICAL, ArchiveMatchChannel.MESSAGE_HISTORY),
    }
    assert all(not item.gap_codes for item in foreign_saturated)
