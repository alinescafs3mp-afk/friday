...
from friday.orchestration.coding_result_archive_pack_admission import build_coding_result_archive_pack_admission
...
def archive_facts() -> tuple[object, object, object, object]:
    ...
    publication = build_coding_result_publication_admission("publication-1", "turn-1", plan, pack, uncertainty=uncertainty)
...
def test_empty_text_file_and_archive_carriers() -> None:
    ...
    assert build_coding_mode_carrier("carrier-1", "turn-1", archive_plan=empty_plan).state is CodingModeCarrierState.TEXT
    ...
    publication = build_coding_result_publication_admission("publication-1", "turn-1", plan, uncertainty=uncertainty)
...
def test_unknown_uncertainty_and_blocked_publication_are_not_carriers() -> None:
    ...
    assert result.reason in {CodingModeCarrierReason.PUBLICATION_BLOCKED, CodingModeCarrierReason.UNCERTAINTY_UNKNOWN}
