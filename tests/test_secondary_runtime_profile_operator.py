...
def test_operator_builds_one_runtime_loadable_candidate(tmp_path: Path) -> None:
    ...
    assert "--enable-deterministic-inference" not in profile.server_arguments("a" * 64)
...
@pytest.mark.parametrize("patch_sha256", ["0" * 64, "D" * 64, "d" * 63])
def test_operator_rejects_invalid_sglang_compat_patch_identity(
    tmp_path: Path,
    patch_sha256: str,
) -> None:
    operator = importlib.import_module("runtime_profile_operator")
    args = _candidate_args(tmp_path)
    args.sglang_compat_patch_sha256 = patch_sha256

    with pytest.raises(operator.ProfileOperatorError, match="compatibility patch identity"):
        operator.build_candidate(args)
...
def test_sglang_compat_patch_identity_changes_the_engine_binding(tmp_path: Path) -> None:
    operator = importlib.import_module("runtime_profile_operator")
    ...
    second_args.sglang_compat_patch_sha256 = "e" * 64
...
def test_endpoint_identity_accepts_only_the_closed_candidate_specific_surface(
    ...
    candidate = json.loads(args.output.read_text(encoding="utf-8"))
    candidate["deterministic_inference_enabled"] = True
    binding = operator._engine_sha256(candidate)
    candidate["engine_binding_sha256"] = binding
    candidate["profile_id"] = f"gptoss20b-{binding}"
    candidate["served_model_alias"] = f"friday-secondary-{candidate['profile_id']}"
...
def test_manual_counter_only_product_sidecars_are_rejected(tmp_path: Path) -> None:
    ...
    common = {
        ...
        "primary_api_key_file": tmp_path / "primary.key",
        "product_output": tmp_path / "product.json",
        "output": tmp_path / "node.json",
    }

    with pytest.raises(live.LiveFailureBatteryError, match="manual counter-only product witness"):
        live.begin_physical_observation(
            **common,
            api_key_file=tmp_path / "gateway.key",
            primary_pid=2613,
        )
    with pytest.raises(live.LiveFailureBatteryError, match="manual counter-only product witness"):
        live.record_physical_power_loss(
            **common,
            ...
            product_state_path=tmp_path / "product-begin.json",
    ...
    with pytest.raises(live.LiveFailureBatteryError, match="manual counter-only product witness"):
        live.finish_physical_observation(
            **common,
            ...
            product_state_path=tmp_path / "product-off.json",
