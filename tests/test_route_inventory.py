...
#: +2 к 179: ночные сводки о поведении системы за сутки (заказ владельца
#: 2026-08-04). `GET /api/compacts` — список прогонов для вкладки в панели,
#: `POST /api/compacts/run` — собрать за названные сутки по кнопке. Второй нужен
...
# 197 → 199: owner-only issue/consume for the independently signed, one-use
# semantic-supervisor representative-window witness.
EXPECTED_OPERATIONS = 199
