...
class WorkerSupervisor:
    ...
    def _initial_delay(self, task: IntervalTask) -> float:
        ...
        #
        # Откат системных часов после записи `last_finished_at` делает elapsed
        # отрицательным. Без верхней границы суточная задача с отметкой на
        # месяц вперёд спала месяц плюс обычный интервал. Будущая настенная
        # отметка не даёт права запускать воркер сразу, но и отложить его может
        # не больше одного его нормального интервала.
        return max(0.0, min(task.interval_sec, task.interval_sec - elapsed))
