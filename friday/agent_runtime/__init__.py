...
_WEB_REQUEST_VERB = (
    r"найд[иу]\w*|найти|поищ\w*|поиш\w*|ищи|искать|посмотр\w*|смотри|глян\w*|"
...
_WEB_PLACE = (
    ...
    r"в\s+интер\w{0,2}н[еэ]т\w*(?!-)|в\s+инете|в\s+сети\b|в\s+вебе|в\s+гугле|в\s+яндексе|"
    r"в\s+google|в\s+интернет-поиске"
)
...
_ASKS_FOR_THE_WEB = re.compile(
    r"(?:^|[.!?]\s+)\s*(?:а\s+|и\s+|ну\s+|пожалуйста[,\s]+)*"
    r"(?:"
    # «найди в интернете …», «посмотри в сети …»
    rf"(?:{_WEB_REQUEST_VERB})[^.!?]{{0,40}}?(?:{_WEB_PLACE})|"
    # «в интернете найди …» — обратный порядок, но всё ещё начало просьбы
    rf"(?:{_WEB_PLACE})[^.!?]{{0,20}}?(?:{_WEB_REQUEST_VERB})|"
    # «погугли …» — глагол сам называет место
    r"(?:погугли|загугли|нагугли)\w*|"
