...
_LOOKS_LIKE_A_QUESTION = re.compile(
    r"^\s*(?:пятниц\w*[,\s]+)?(?:а|и|ну|слушай|скажи|подскажи)?[,\s]*"
    r"(?:что|кто|где|когда|сколько|как\w*|почему|зачем|чем|куда|откуда|"
    r"расскажи|объясни|напомни|покажи|дай|нужн\w*|интересно)\b",
    re.IGNORECASE,
)
...
class AgentRuntime:
        ...
        *,
        ...
        if context.small_talk or looking_outward:
            ...
            try:
                ...
                scored = [item for item in found if item.get("score") is not None]
                ...
                if found and len(scored) == len(found) and not any(
                    float(item["score"]) > _NOISE_FLOOR for item in scored
                ):
                    LOGGER.info(
    ...
    async def _web_query_by_arbiter(self, message: str) -> str | None:
                ...
                [
                    {
                        ...
                        "content": (
                            ...
                            '{"вид": "интернет|архив|материал|другое", "запрос": "строка для поисковика"}.\n'
                            "интернет — спрашивают о внешнем мире: новости, погода, курсы, цены, "
                            "факты о людях и технологиях, документация, «что такое», «когда вышел».\n"
        ...
        if not isinstance(verdict, dict):
            return None
        if "интернет" not in str(verdict.get("вид") or "").strip().casefold():
            return None
    ...
    async def _prefetch_the_web_if_asked(
        ...
        # базе и нашёлся обычным поиском. Если контекст уже что-то нашёл, вопрос
        # был про своё; прямая просьба «найди в интернете» это правило снимает.
        if not asked_outright and context is not None and context.knowledge_hits:
            return
        ...
        query = await self._web_query_by_arbiter(message)
        if not query:
