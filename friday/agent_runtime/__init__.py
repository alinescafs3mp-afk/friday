...
class AgentRuntime:
                ...
                [
                    {
                        ...
                        "content": (
                            ...
                            "знание — ответ УСТОЯЛСЯ и ты его знаешь: история, география, наука, "
                            "определения, «кто был вторым президентом США», «когда началась Вторая "
                            "мировая», «что такое консенсус Raft», «сколько дней в феврале 2028». "
                            "Такой факт не меняется, и искать его в интернете незачем.\n"
    ...
    async def _prefetch_the_web_if_asked(
        ...
        kind, query = await self._web_query_by_arbiter(message)
