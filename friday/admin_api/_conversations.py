...
@router.get("/chats/cursor")
async def chat_feed_cursor(request: Request) -> dict[str, Any]:
    """Изменилась ли лента переписки. Дёшево, чтобы спрашивать часто.

    Заказ владельца: вкладка «Переписка» должна обновляться сама, «как настоящий
    телеграм», а не по F5. Опрашивать ради этого саму ленту нельзя — это
    оконная функция по всем сообщениям плюс четыре подзапроса; здесь два
    агрегата по индексированным столбцам.

    В след НЕ пишется, в отличие от чтения самой ленты: отпечаток не содержит ни
    строчки чужой переписки — только число сообщений и время последнего. Иначе
    журнал доступа к личным данным заполнился бы записями «посмотрел, не
    изменилось ли» по одной каждые несколько секунд, и настоящее чтение в нём
    утонуло бы.
    """
    _require(request, "admin.all_data.read")
    return _services(request).storage.chat_feed_cursor()
