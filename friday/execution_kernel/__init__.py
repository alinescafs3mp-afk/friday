...
def _person_answer_for_llm(answer: dict[str, Any], *, zone: Any) -> dict[str, Any]:
    ...
    if not messages and not files:
        ...
        trimmed["про загруженные материалы"] = (
            "в общем архиве материалы не разделены по людям — кто именно их загрузил, "
            "система не записывает. Не отвечай, что человек ничего не присылал: этого "
            "никто не проверял. Скажи, что по загрузкам ответа нет."
        )
    return trimmed
