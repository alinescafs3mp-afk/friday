...
# (docx tables, xlsx and csv/tsv in `friday.documents` all render this shape).
...
# the list. Configurable because the number belongs to the model, not to Friday.
#
...
def chunk_scheme(settings: FridaySettings) -> str:
...
# Сколько «этот документ уже доставали» стоит в итоговой оценке. Ноль по замеру —
# см. развёрнутое объяснение у места использования. Оставлено именованной
# константой, чтобы вернуть сигнал можно было ОДНИМ осознанным числом, а не
# правкой формулы.
_USAGE_WEIGHT = 0.0
...
class HybridSearcher:
    ...
    def __init__(
        ...
        *,
        ...
        # `FRIDAY_RETRIEVAL_DENSE_EVIDENCE_MIN` in docs/ARCHITECTURE.md §7.
    ...
    async def search(
        ...
        *,
        ...
        #
        # Умолчание `False` — не осторожность, а замер: на 20 документных эталонах
        # живого архива расширение роняло recall@10 0.35 -> 0.15 и стоило +556 мс.
        # Прежнее умолчание `True` означало, что политику принимает тот, кто
        # ПРОМОЛЧАЛ, и молчали ровно ошибавшиеся дороги: из семи вызывающих четыре
        # объявляли политику явно, а три (`/api/search`, admin `eval_search` и
        # синтетический `retrieval_bench`) включали граф, не спросив. Теперь
        # молчание означает измеренное поведение, а обратное надо объявить вслух —
        # и это объявление видно в ревью.
        ...
        # Вес 0.0 — ЗАМЕРЕННЫЙ, а не забытый. «Этот документ уже доставали раньше»
        # оказалось не подсказкой, а самоподкрепляющейся петлёй: сигнал поднимает
        # то, что и так поднимается, а на архиве владельца верх занимают документы-
        # концентраторы («Судимости.docx», «люди (35 человек).docx») — они похожи
        # на всё сразу и потому извлекаются чаще всех.
        #
        ...
        #
        # Эффект пограничный и РАЗНОНАПРАВЛЕННЫЙ: без сигнала на один эталон больше
        # попадает в десятку, с сигналом найденное стоит чуть выше. Выбран recall:
        # «нашлось вообще» важнее «нашлось на строчку выше», а разница в MRR внутри
        # десятки человеку почти не видна.
        #
        # Прежняя редакция этого комментария утверждала, что сигнал ВРЕДИТ с
        # p=0.0044 по MRR. Это число получено на сборке БЕЗ переранжировщика —
        # `run_eval` собирал поисковик вручную и половину настроек не передавал, —
        # то есть относилось к поиску, которого у человека нет. Оставлено здесь
        # намеренно: замер на неверной конфигурации выглядит точно так же
        # убедительно, как верный.
        #
        # Счётчик по-прежнему пишется и показывается в панели («Использований в
        # поиске») — он честная статистика, просто не аргумент в ранжировании.
        w_usage = 0.0 if "usage" in off else _USAGE_WEIGHT
