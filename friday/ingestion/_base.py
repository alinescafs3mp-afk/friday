...
use them without importing ``friday.ingestion`` — which imports the mixins, and would
...
from friday.storage import FridayStorage, SourceReferenceConflictError, normalize_entity_name
...
from friday.whisper import WhisperUnavailable, looks_like_audio, transcribe_bytes

if TYPE_CHECKING:
    from friday.agent_runtime.llm import LLMRouter
    from friday.knowledge_graph import KnowledgeGraph
# Named for the package, not this module: `__name__` here is "friday.ingestion._base", and
...
#: Служебные слова — ЗАКРЫТЫЙ класс языка, и потому их можно перечислить.
#:
#: Прежний список из 34 слов не содержал ни «при», ни «года», ни «нет», ни «где»,
#: ни «прошу» — и все они стали тегами. Замер на живом архиве владельца (1536
#: объектов, 15 900 экземпляров тегов): 75 из 100 самых частых тегов не несут
#: темы вовсе, а вместе это 8290 экземпляров — 52% всего тегового объёма.
#: Разбивка: «document»/«application» 3048, названия граф бланка 1751, обрывки
#: словосочетаний 1747, обрывки ФИО 901, чистые служебные слова 843.
#:
#: Перечисление здесь законно ровно потому, что предлоги, союзы, частицы и
#: местоимения — конечный список, а не «ещё один шаблон вместо понимания»: новых
#: предлогов в русском языке не появляется. Всё, что открытым классом (темы,
#: имена, названия), решается не списком, а корпусной частотой и графом.
_STOPWORDS = {
    # предлоги
    "без",
    "близ",
    ...
    "уже",
    "ещё",
    # связки и частые служебные глаголы
    "будет",
    "будут",
    "была",
    "были",
    ...
    "являются",
    "является",
    # канцелярские связки: своей темы не несут, а частотны в каждом документе
    # архива. Вид документа теперь отдельный тег («вид:рапорт»), и дублировать
    # его обрывками формулы бланка незачем.
    "прошу",
    "просим",
    "настоящим",
    "согласно",
    ...
    "сведения",
    "перечень",
    # месяцы: закрытый класс, темой не бывают. Дата документа живёт отдельным
    # фактом (`occurred_at`), и дублировать её тегом «января» незачем — на живом
    # архиве он стоял на 118 объектах.
    "января",
    "февраля",
    "марта",
    "апреля",
    ...
    "суток",
    "сутки",
    # след разбора таблиц: извлекатель Excel пишет «--- Sheet: Лист1 ---», и
    # слово уезжало в теги 80 раз. Это разметка носителя, а не тема.
    "sheet",
    "лист1",
    # английские
    "about",
...
#: Как текст режется на слова-кандидаты в теги. ОДНА регулярка на весь путь:
#: отбор ключевых слов и обучение списку бланковых слов обязаны видеть одни и те
#: же токены. Пока их было две, «ф.и.о» проходило мимо ворот — в списке бланка
#: точка не считалась частью слова, а в отборе считалась, и тег стоял на 120
#: объектах живого архива после чистки.
_TAG_TOKEN = re.compile(r"\b[А-ЯЁа-яёA-Za-z][А-ЯЁа-яёA-Za-z0-9._+#-]{2,63}\b")


def tag_tokens(text: str) -> list[str]:
    """Слова-кандидаты в теги, как их видит и отбор, и обучение бланку."""

    return _TAG_TOKEN.findall(text or "")


#: Доля корпуса, выше которой слово перестаёт что-либо отбирать.
#:
#: Замерено на архиве владельца: у 66.3% объектов самый частый тег отбирал больше
#: 10% корпуса, медиана — 181 объект из 1536. Порог 0.15 выбран как наибольший,
#: при котором слово всё ещё сужает выбор хотя бы в семь раз.
#:
#: ⚠️ Одной частотой мусор НЕ отделяется, и это замерено: «рапорт» стоит в 316
#: документах из 1536 (20.6%), а «нет» — в 279 (18%). Первое — тема, второе —
#: служебное слово, и НИКАКОЙ порог их не различает: между ними либо оба
#: остаются, либо оба уходят. Поэтому частота работает только вместе со списком
#: служебных слов, а не вместо него.
#:
#: При этом пороге «рапорт» и «рядовой» (21.7%) в теги не попадают, и это
#: осознанная плата: вид документа теперь отдельный тег (`вид:рапорт`), который
#: отбирает те же 293 документа точнее, чем слово, встречающееся и в ответах на
#: чужие рапорты.
_TAG_CORPUS_SHARE_MAX = 0.15
...
def _extract_keywords(
    ...
    *,
    ...
    """Слова, по которым осмысленно ОТБИРАТЬ документы этого архива.

    Три отсева, и каждый закрывает свой измеренный класс мусора:

    * служебные слова (`_STOPWORDS`) — 843 экземпляра на живом архиве;
    * `blocked` — части имён людей, уже привязанных к документу: «александрович»
      стоял тегом на 194 объектах, «сергей» на 105. Имена знает граф, и тег,
      повторяющий их, не добавляет ни одного нового отбора;
    * корпусная частота — слово, встречающееся почти везде, не сужает ничего.

    Плюс свёртка словоформ: «командиру» (203), «командир» (129), «командира»
    (47), «командования» (38) — это ОДНО понятие, размазанное по четырём тегам,
    и на экране выбора человек видит четыре чипа вместо одного. Побеждает самая
    частая форма — она же чаще всего и есть словарная.
    """
    ...
    ceiling = int(corpus_size * _TAG_CORPUS_SHARE_MAX) if corpus_size else 0
    counts = Counter(
        token
        ...
        and not (ceiling and int((document_frequency or {}).get(token, 0)) > ceiling)
    )
    if not counts:
        return []
    ...
    for token in ranked:
        root = stem(token) or token
        if root in seen_stems:
            continue
        seen_stems.add(root)
        kept.append(original_form[token].casefold())
...
# Слово имени собственного. Вторая ветвь — для аббревиатур и кодов («ГПТУ»,
# «АВ-690234», «RTX»), и она НЕ принимает чисто числовой токен.
#
# Замерено на архиве владельца: без этого запрета к названию города прилипало
# число из соседней строки — «Волгоград 17», «Самара 17», «Ханты-Мансийск 17»,
# «Тюмень 89224794862». Семнадцать — это номер СЛЕДУЮЩЕГО пункта анкеты
# («17. Гражданство»), а одиннадцатизначное — телефон: правило ищет «город» плюс
# до трёх слов, а `\s+` спокойно перешагивает перевод строки. В графе так
# завелось двадцать узлов-двойников настоящих городов, и на них же пришлись обе
# ошибки в замере точности падежного сопоставления.
#
# Проверка перед правкой, 200 документов живого корпуса: различных имён было
# 3620, стало 3615. Пропали ровно шесть — все шесть с прилипшим числом; ни одного
# законного имени не потеряно. Номер дома, индекс, телефон и номер пункта стоят
# РЯДОМ с названием, но частью названия не являются.
_PROPER_TOKEN = r"(?:[A-ZА-ЯЁ][\w.+#&/-]{1,40}|(?![0-9]+\b)[A-ZА-ЯЁ0-9]{2,24})"
...
# `-кевич` — БЕЛОРУССКАЯ ФАМИЛИЯ, а не отчество, и это не мелочь.
#
# Отчество образуется от имени: Александр → Александрович, Дмитрий → Дмитриевич.
# Имён, оканчивающихся на «к», в обиходе нет, поэтому суффикса «-кевич» у отчества
# не бывает — зато он даёт целый пласт фамилий: Костюкевич, Янушкевич, Мицкевич.
# Пока «Костюкевич» считался отчеством, правило читало строку ведомости
# «Рядовой Костюкевич Кирилл Михайлович» задом наперёд — как «Имя Отчество Фамилия»,
# начиная со звания, — и заводило человека по имени «Рядовой Костюкевич Кирилл»,
# останавливаясь ровно перед настоящим отчеством. Настоящий человек при этом
# не появлялся вовсе: разбор уже съел его имя.
_PATRONYMIC = r"[А-ЯЁ][а-яё]+(?:ович|(?<!к)евич|ьевич|овна|евна|ьевна|инична|ична)"
# Слова, которыми к человеку ОБРАЩАЮТСЯ или называют его звание. В первой ветке они
# попадают в слот фамилии: «Уважаемая Снежана Николаевна» заводилась как человек по
# фамилии «Уважаемая» (девять таких на живом архиве). Это запрет в самой регулярке,
# а не отсев после: движок обязан отступить и найти настоящее имя правее — иначе
# «Рядовой Иван Петрович Сидоров» потеряет Сидорова вместе со званием.
_ADDRESS_OR_RANK = (
    "Уважаемый|Уважаемая|Уважаемые|Товарищ|Гражданин|Гражданка|Господин|Госпожа|"
    "Дорогой|Дорогая|Рядовой|Ефрейтор|Сержант|Старшина|Прапорщик|Лейтенант|Капитан|"
    "Майор|Подполковник|Полковник|Генерал|Матрос|Курсант|Солдат"
)
# ФАМИЛИЯ В ПЕРВОЙ ВЕТКЕ МОЖЕТ БЫТЬ НАБРАНА ЗАГЛАВНЫМИ, и это не косметика.
#
# В служебных документах фамилию печатают капслоком: «ЛЯПИН Александр
# Александрович», «МОИСЕЕВ Алексей Алексеевич». Первая ветка требовала
# `[А-ЯЁ][а-яё-]` и такое имя не брала — зато срабатывала ВТОРАЯ, «Имя Отчество
# Фамилия», и приклеивала к человеку следующее слово таблицы:
#
...
#
# Замерено на всех 1536 живых документах, обе регулярки прогнаны рядом: обрывков
# вида «Имя Отчество Хвост» 163 → 5, различных ФИО 4376 → 8988, упоминаний
# 21461 → 45164. Верных имён не теряется НИ ОДНОГО — правило просто перестаёт
# спотыкаться о регистр. Дублей не будет: `normalize_entity_name` складывает
# «ИГНАТОВ Василий Владимирович» и «Игнатов Василий Владимирович» в один ключ.
#
# Цена бездействия там же: из 4735 различных ФИО с заглавной фамилией 2571 (9891
# упоминание) не имеют в графе НИ ОДНОГО узла. Правило не просто плодило мусор —
# оно отдавало мусор ВМЕСТО человека.
_PERSON_FULL_NAME_RE = re.compile(
    rf"(?<![\w-])((?:(?!(?:{_ADDRESS_OR_RANK})(?![\w-]))"
    rf"[А-ЯЁ][А-ЯЁа-яё-]{{2,30}}\s+[А-ЯЁ][а-яё-]{{2,30}}\s+{_PATRONYMIC})"
    rf"|(?:(?!(?:{_ADDRESS_OR_RANK})(?![\w-]))"
    rf"[А-ЯЁ][а-яё-]{{2,30}}\s+{_PATRONYMIC}\s+[А-ЯЁ][а-яё-]{{2,30}}))(?![\w-])"
)
#: Слово с заглавной буквы непосредственно ПЕРЕД совпадением второй ветки.
#: Значит фамилия уже была, и «Имя Отчество Хвост» — склейка через границу ячейки.
#:
#: Латиница и цифры внутри слова — не описка, а СКАН. Фамилии в ведомостях приходят
#: через распознавание, и в них подмешаны латинские буквы: «Cеверинов» с латинской C,
#: «ПОЛИКАПИII» с латинскими I. Прежний страж требовал слово целиком из кириллицы,
#: об эти две фамилии спотыкался и пропускал в граф «Олег Валерьевич Командир» и
#: «Алексей Александрович Водитель» — должность соседней ячейки вместо фамилии.
_NAME_PRECEDED_BY_A_SURNAME = re.compile(r"[A-ZА-ЯЁ][A-Za-zА-Яа-яЁё0-9-]{2,30}\s*$")
_PERSON_RE = re.compile(
    r"(?<![\w-])((?:[А-ЯЁ][а-яё-]{2,30}|[A-Z][a-z-]{2,30})\s+"
    r"(?:[А-ЯЁ][а-яё-]{2,30}|[A-Z][a-z-]{2,30}))(?![\w-])"
)
# После «город N» в русском тексте идёт ОСТАЛЬНОЙ АДРЕС, а не остаток названия.
#
# Хвост «до трёх слов» съедал его целиком: «Кропоткин СНТ Мичурина», «Кемерово СОШ
# Ягуновская», «Ишим Тюменская», «Новокузнецк Кемеровская». Замерено на 1562 живых
# документах: многословных имён восемь, годных среди них НОЛЬ; при переходе на одно
# слово число упоминаний не меняется (274), различных имён 42 → 37, и три города —
# Астрахань, Ишим, Новокузнецк — впервые появляются под своим настоящим именем.
# По-английски порядок другой: «city of New York» — название и есть два слова,
# а штат отделён запятой, поэтому английским формам хвост оставлен.
_LOCATION_EXPLICIT_RE = re.compile(
    rf"(?:(?i:\b(?:город(?:е|а)?|регион(?:е|а)?|локация))\s+({_PROPER_TOKEN})"
    rf"|(?i:\b(?:city\s+of|town\s+of|location))\s+({_PROPER_TOKEN}(?:\s+{_PROPER_TOKEN}){{0,2}}))",
    re.UNICODE,
)
...
DECLARED_ENTITY_METHODS = frozenset(
    {
        ...
        # То же правило — «имя узла названо в этом тексте», — когда имя стоит в
        # косвенном падеже или разорвано разметкой таблицы. Падежи складываются
        # ТОЙ ЖЕ пословной свёрткой, которой вычисляется `normalized_name`, то
        # есть которой уже определяется тождество самого узла: «Кублику
        # Александру Юрьевичу» и «Кублик Александр Юрьевич» — один узел, а
        # значит и одно упоминание.
        #
        # Замерено на архиве владельца перед включением: 80 документов, 89 новых
        # привязок, из выборки в 40 верны 38. Обе ошибки — на одной заведомо
        # мусорной сущности («БОЕВОЙ ПОДГОТОВКИ», обрывок фразы, заведённый как
        # организация), то есть цена метода, а не самого сопоставления.
        # Неожиданное большинство находок — вовсе не падежи, а имена, разорванные
        # переводом строки при выгрузке таблиц: «ЮНУСОВ\nУмарали\nУсманович»
        # буквальным поиском не находится ни в одной форме.
        "existing_canonical_name_inflected",
        "existing_entity_inflected_mention",
...
def _extract_entities(text: str) -> list[dict[str, Any]]:
    ...
    for match in _LOCATION_EXPLICIT_RE.finditer(text):
        add(match.group(1) or match.group(2), EntityType.LOCATION, 0.9, "explicit_location_marker")
    ...
    for match in _INFRA_RE.finditer(text):
        # «Сервер» — ещё и имя человека (крымскотатарское), и правило читало его как
        # слово «сервер», а следующее слово — как имя машины. Так в графе завёлся узел
        # «Викторович»: восемь документов, и все — про людей. Отчество не бывает
        # именем сервера ни при каком написании, поэтому такой захват отвергается.
        #
        # Проверять надо ПЕРВОЕ СЛОВО захвата, а не весь захват: правило берёт до трёх
        # слов, и на живом архиве отчество попалось одиноким лишь потому, что дальше
        # шла запятая. «Сервер Викторович Аблаев» отдал бы «Викторович Аблаев» —
        # отчество вместе с фамилией, и проверка целиком его бы не узнала.
        infra_head = match.group(1).split()[0] if match.group(1).split() else ""
        if re.fullmatch(_PATRONYMIC, infra_head):
            continue
    ...
    for match in _PERSON_FULL_NAME_RE.finditer(text):
        ...
        # Хвост, приклеенный к УЖЕ НАЗВАННОМУ человеку. Вторая ветка читает
        # «Имя Отчество Фамилия», и в таблице личного состава это оказывается
        # концом одной записи и началом соседней ячейки: «…КУШНАЗАРОВ Анатолий
        # Олегович | Вакансия». Если слева от совпадения стоит слово с заглавной,
        # фамилия уже была — значит перед нами склейка через границу ячейки, а не
        # человек по имени «Анатолий Олегович Вакансия».
        # Отчество у человека ОДНО. Два подряд — значит имя собрано из разных ячеек
        # или искажено распознаванием: «Большаков Константинович Александрович»,
        # «Букатин Дмитриевич Петрович», «Колодяжный Иванович Юрьевич» — все пять
        # таких на живом архиве стоят в ведомостях поодиночке в клетке, и настоящего
        # имени в них не восстановить. Порядок «Имя Отчество Фамилия» с фамилией на
        # «-евич» под запрет не попадает: «-кевич» отчеством больше не считается,
        # поэтому у «Кирилл Михайлович Костюкевич» отчество ровно одно.
        if len(words) == 3 and re.fullmatch(_PATRONYMIC, words[1]) and re.fullmatch(_PATRONYMIC, words[2]):
            continue
        if len(words) == 3 and re.fullmatch(_PATRONYMIC, words[1]):
            before = text[max(0, match.start(1) - 40) : match.start(1)]
            if _NAME_PRECEDED_BY_A_SURNAME.search(before):
                continue
...
def _storage_relative(root: Path, path: Path) -> str:
    ...
    архив к машине. Абсолютный путь означает, что после переезда, смены FRIDAY_HOME
...
def _extracted_text_digest(text: str) -> str:
    """Отпечаток ИЗВЛЕЧЁННОГО текста — ключ «тот же документ», а не «тот же файл».

    Пробелы схлопываются: разница в переносах строк между экспортом из Word и из
    PDF — не разница в документе. Регистр НЕ сбрасывается — «Приказ №214» и
    «ПРИКАЗ №214» это разные написания, и объявлять их одним документом здесь
    нельзя; для таких пар существует очередь разбора, где решает человек.

    Пустой текст даёт пустой отпечаток, а не хеш пустой строки: у картинки и у
    нечитаемого файла текста нет у всех сразу, и общий ключ склеил бы в один
    документ всё, что не разобралось.
    """
...
def _parse_model_response(response: dict[str, Any], *, what: str = "Model advice") -> dict[str, Any]:
    ...
    FRIDAY_COGNITION_MAX_TOKENS — на пути зрения это была ложная подсказка, пока
    ...
    if finish == "length":
        ...
        raise ValueError(
            ...
            " — raise FRIDAY_COGNITION_MAX_TOKENS"
        )
