...
failure modes that matter, and the report says which of them Friday would survive.
...
def _probe_embedding_batch(
    client: httpx.Client, settings: FridaySettings, api_key: str, report: ModelReport
) -> None:
    ...
    requests Friday makes: the indexer batches up to
    ``FRIDAY_EMBEDDINGS_MAX_INPUTS_PER_REQUEST`` inputs per call. Measured against a
    ...
    report.probes.append(
        Probe(
            ...
            "— уменьшите FRIDAY_EMBEDDINGS_MAX_INPUTS_PER_REQUEST",
            elapsed,
        )
    )


def check_model(settings: FridaySettings, *, timeout: float = 60.0) -> ModelReport:
    ...
    with httpx.Client(timeout=timeout, trust_env=False) as client:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                hint = " (set FRIDAY_LLM_API_KEY)" if response.status_code == 401 else ""
        ...
        # 2. Does it answer at all, inside a budget Friday actually uses?
        ...
        if settings.embeddings_enabled and not settings.embeddings_model.strip():
            report.probes.append(
                Probe(
                    "embeddings",
                    False,
                    "FRIDAY_EMBEDDINGS_MODEL is empty: dense recall is off despite "
