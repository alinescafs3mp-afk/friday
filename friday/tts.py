"""Local text-to-speech — Friday speaks a reply on request within a conversation.
...
``FRIDAY_TTS_ENABLED`` — the core system runs, and the agent's ``speak`` tool
stays unavailable, when it is absent or disabled. See :mod:`friday.whisper` for
