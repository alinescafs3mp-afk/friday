...
renderers.users=async gen=>{await loadUsers(gen);const rows=state.users.map(u=>{
  const remove=!state.hardDeleteEnabled||u.preset_key==='owner'?'':` <button class="btn small danger" ${call('deleteUserDialog',u.id)}>Удалить</button>`;
  return `<tr><td><b>${esc(u.display_name||u.username||u.id)}</b><div class="mono">${esc(u.id)}</div><div class="muted">${esc(u.source)} ${esc(u.external_id)}</div></td><td><span class="badge ${u.status==='active'?'ok':'bad'}">${esc(u.status)}</span></td><td><select class="field" ${chg('setPreset',u.id)}>${state.presets.map(p=>`<option value="${esc(p.preset_key)}" ${u.preset_key===p.preset_key?'selected':''}>${esc(p.name||p.preset_key)}</option>`).join('')}</select></td><td><select class="field" ${chg('setSupervisor',u.id)}><option value="">— никому не подчинён —</option>${state.users.filter(o=>o.id!==u.id).map(o=>`<option value="${esc(o.id)}" ${supervisorOf(u)===o.id?'selected':''}>${esc(o.display_name||o.username||o.id)}</option>`).join('')}</select></td><td>${fmtDate(u.last_seen_at)}</td><td><button class="btn small" ${call('permissionDialog',u.id)}>Права</button> <button class="btn small ${u.status==='active'?'danger':'good'}" ${call('setUserStatus',u.id,u.status==='active'?'disabled':'active')}>${u.status==='active'?'Отключить':'Включить'}</button>${remove}</td></tr>`
});const deletionNotice=state.hardDeleteEnabled?'':`<div class="notice">Безвозвратное удаление временно недоступно: старая резервная копия может восстановить учётную запись. Аккаунт можно безопасно отключить.</div>`;setApp(gen,`<section class="card"><div class="toolbar"><h2 class="grow">Пользователи и роли</h2><button class="btn" ${call('createUserDialog')}>Добавить</button><span class="badge">${Number(state.usersTotal||rows.length)}</span></div>${deletionNotice}${rows.length?table(['Пользователь','Статус','Пресет','Подчинён','Последняя активность','Действия'],rows):empty('Пользователи не найдены')}</section>`)};
...
actions.deleteUserDialog=async id=>{try{
  const plan=await api(`/api/admin/users/${q(id)}/deletion`);state.userDeletionPlan=plan;
  const user=state.users.find(item=>item.id===id)||{};const name=user.display_name||user.username||id;
  const retained=Number(plan.retained?.audit_log||0);const blocked=(plan.blockers||[]);
  const auditNote=retained?`<div class="muted">Append-only журнал аудита (${retained}) сохраняется как след действий, но доступ, переписка и принадлежащие аккаунту строки в него не переносятся.</div>`:'';
  if(blocked.length){
    const reasons=blocked.map(item=>`<li>${esc(item.message)}${Number(item.count||0)>1?` <span class="badge">${Number(item.count)}</span>`:''}</li>`).join('');
    openModal(`Удаление: ${name}`,`<div class="notice"><b>Удаление сейчас заблокировано безопасной проверкой.</b><ul>${reasons}</ul></div>${auditNote}`,`<button class="btn" ${call('closeModal')}>Закрыть</button>`);return;
  }
  openModal(`Удалить учётную запись: ${name}`,`<div class="notice"><b>Необратимо в текущей базе.</b> Будут отозваны способы входа и удалены ${Number(plan.planned_delete_rows||0).toLocaleString('ru')} строк учётки. Тот же ID нельзя будет создать повторно.</div>${auditNote}<div class="muted">Резервная копия, сделанная до удаления, может восстановить аккаунт. Операция не стирает физические следы из append-only аудита, WAL, старых резервных копий и ранее созданных экспортов.</div><div class="form"><label>Для подтверждения введите точный ID <span class="mono">${esc(id)}</span><input id="deleteUserConfirmation" autocomplete="off" spellcheck="false"></label></div>`,`<button class="btn" ${call('closeModal')}>Отмена</button><button class="btn danger" ${call('deleteUser',id)}>Удалить учётную запись</button>`);
  setTimeout(()=>document.getElementById('deleteUserConfirmation')?.focus(),0)
}catch(e){toast(e.message,true)}};
actions.deleteUser=async id=>{const plan=state.userDeletionPlan;if(!plan||plan.user_id!==id){toast('Повторите предварительную проверку',true);return}const typed=document.getElementById('deleteUserConfirmation')?.value||'';if(typed!==id){toast('ID введён не точно — удаление отменено',true);return}try{const out=await api(`/api/admin/users/${q(id)}`,{method:'DELETE',body:JSON.stringify({confirmation:typed,fingerprint:plan.fingerprint})});closeModal();state.userDeletionPlan=null;toast(`Пользователь удалён, строк: ${Number(out.deleted_rows||0)}`);await loadUsers();refresh()}catch(e){toast(e.message,true)}};
...
// --- Внешние источники: базы, в которые Пятница ходит ЧИТАТЬ ------------
// Строки подключения на этом экране нет и быть не может: объявляется имя
// переменной окружения. Поэтому здесь же показывается, задана ли она — иначе
// источник выглядит рабочим, а первый запрос падает непонятной ошибкой.
renderers.sources=async gen=>{
  const uid=selectedUser();
  const data=await api(`/api/admin/data-sources?user_id=${q(uid)}`);
  if(gen!==renderGen)return;
  const rows=(data.sources||[]).map(s=>`<tr><td><b>${esc(s.name)}</b><div class="muted">${esc(s.description||'')}</div></td><td><span class="badge">${esc(s.kind)}</span></td><td class="mono">${esc(s.dsn_env)}</td><td><span class="badge ${s.secret_present?'ok':'warn'}">${s.secret_present?'переменная задана':'переменная не задана'}</span></td><td>${fmtDate(s.last_used_at)}</td><td><button class="btn small" ${call('sourceSchema',s.name)}>Схема</button> <button class="btn small danger" ${call('forgetSource',s.name)}>Забыть</button></td></tr>`);
  setApp(gen,`<section class="card"><div class="toolbar"><h2 class="grow">Внешние базы-источники</h2><button class="btn primary" ${call('declareSourceDialog')}>Объявить источник</button><span class="badge">${rows.length}</span></div><div class="notice">Пятница ходит в такую базу <b>только читать</b>: разрешён один SELECT, запись отвергается. Строка подключения здесь не хранится — задаётся переменная окружения на сервере, а в базе живёт только её имя.</div>${rows.length?table(['Источник','Вид','Переменная','Секрет','Последнее обращение','Действия'],rows):empty('Источники не объявлены')}</section>`);
};
actions.declareSourceDialog=()=>openModal('Новый источник',`<div class="form"><label>Имя<input id="srcName" placeholder="hr"></label><label>Вид<select id="srcKind" class="field"><option value="postgres">postgres</option><option value="mysql">mysql</option><option value="sqlite">sqlite</option></select></label><label>Переменная окружения со строкой подключения<input id="srcEnv" placeholder="HR_DSN"></label><label>Описание<input id="srcDesc" placeholder="кадровая база склада"></label><div class="notice">Саму строку подключения сюда вводить не нужно и нельзя: задайте её переменной окружения на сервере и перезапустите службу.</div></div>`,`<button class="btn" ${call('closeModal')}>Отмена</button><button class="btn primary" ${call('declareSource')}>Объявить</button>`);
actions.declareSource=async()=>{try{await api('/api/admin/data-sources',{method:'POST',body:JSON.stringify({user_id:selectedUser(),name:document.getElementById('srcName').value.trim(),kind:document.getElementById('srcKind').value,dsn_env:document.getElementById('srcEnv').value.trim(),description:document.getElementById('srcDesc').value.trim()})});closeModal();toast('Источник объявлен');refresh()}catch(e){toast(e.message,true)}};
actions.forgetSource=async name=>{try{await api(`/api/admin/data-sources/${q(name)}?user_id=${q(selectedUser())}`,{method:'DELETE'});toast(`Источник «${name}» забыт`);refresh()}catch(e){toast(e.message,true)}};
actions.sourceSchema=async name=>{try{const d=await api(`/api/admin/data-sources/${q(name)}/schema?user_id=${q(selectedUser())}`);const tables=d.tables||{};const names=Object.keys(tables);const body=names.length?names.map(t=>`<details><summary>${esc(t)} <span class="muted">(${tables[t].length})</span></summary><div class="pre">${tables[t].map(c=>`${esc(c.column)} — ${esc(c.type)}`).join('\n')}</div></details>`).join(''):empty('В источнике нет таблиц');openModal(`Схема источника «${name}»`,`${d.truncated?'<div class="notice">Описание схемы обрезано по потолку строк: таблиц больше, чем показано.</div>':''}${body}`,`<button class="btn" ${call('closeModal')}>Закрыть</button>`)}catch(e){toast(e.message,true)}};
