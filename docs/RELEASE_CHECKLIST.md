...
- release принимается и первый раз запускается без `FRIDAY_SECONDARY_LLM_*`:
  health имеет `status=ok`, `version=0.207.95`, `secondary.mode=disabled`,
...
- final startup health имеет `status=ok`, `version=0.207.95`, configured/installed
...
и exact `0.207.90/schema50` как immutable fallback. Исторический переход
