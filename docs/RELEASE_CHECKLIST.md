...
- release принимается и первый раз запускается без `FRIDAY_SECONDARY_LLM_*`:
  health имеет `status=ok`, `version=0.207.89`, `secondary.mode=disabled`,
...
- final startup health имеет `status=ok`, `version=0.207.89`, configured/installed
...
- `0.207.85`–`0.207.88` не активировались; исправляющий `0.207.89/schema50` идёт напрямую
...
- pre-activation bootstrap публикует только v1-generation `0.207.84`; после
  активации `0.207.89` не публикуется вторая v1-generation, а `older` остаётся
...
`0.207.89/schema50` один exact `0.207.84/schema50` является одновременно previous
