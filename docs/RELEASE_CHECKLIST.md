...
- release принимается и первый раз запускается без `FRIDAY_SECONDARY_LLM_*`:
  health имеет `status=ok`, `version=0.208.61`, `secondary.mode=disabled`,
...
- final startup health имеет `status=ok`, `version=0.208.61`, configured/installed,
...
- `0.208.57/schema50` принимает exact `0.208.56` как previous и сохраняет
  `0.207.90` immutable fallback;
- для кандидата `0.208.61/schema50` previous и совместимый immutable fallback —
