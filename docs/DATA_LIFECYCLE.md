...
По умолчанию (`FRIDAY_INGESTION_REVIEW_POLICY=unless_explicit`) действует инвариант «Inbox before canonical»: прямое продвижение остаётся только у явного намерения (`/note`/`force_knowledge`, «запомни»/«сохрани»), всё остальное ждёт человека. `assessed` оставлен как явный режим совместимости: при нём вывод `promote` классификатора канонизируется сразу. `always` не продвигает ничего, включая явные сохранения.

**Загрузка файла больше не считается высказыванием о содержимом.** Она остаётся явным ДЕЙСТВИЕМ, но стостраничный docx человек не читал так же, как не читал вставленный абзац, — и раньше именно файл шёл мимо ревью, а абзац в него попадал. Замер на стенде: 342 документа из 344 стали знаниями, не будучи просмотренными. Теперь обе дороги загрузки — подписанное Telegram-вложение и `/api/files` — идут в Inbox по безопасной политике по умолчанию.
...
**Markdown-vault — опциональная plaintext-проекция, а не архив.** Безопасный `FRIDAY_MEMORY_VAULT_MODE=disabled` не создаёт `MemoryVault`, не запускает projector и не удаляет молча прежние файлы. Только явный `full_owner` пишет живые объекты и после полного privacy-filtered обхода убирает stale-проекции. `status`/`doctor` не открывают тела и не публикуют имена, пути или тела legacy-артефактов. Обычный purge не является bulk-remediation: он удаляет только final/crash-temp конкретного purged Knowledge Object.
Все упоминания vault sync/prune ниже относятся только к явно включённому `full_owner`; в `disabled` этого конвейера нет.
...
5. согласованная очистка raw file, точной optional/legacy vault copy, versions, knowledge graph links и backups.
...
До DB commit purge дескрипторно удаляет точно совпавшие optional/legacy final и crash-temp vault-файлы и подтверждает результат directory `fsync`. Последняя проверка live-state, filesystem cleanup и DB delete проходят под одним SQLite `BEGIN IMMEDIATE`; full-owner projector берёт ту же writer-boundary, заново читает KO и держит её до durable note write. Поэтому stale page либо успевает записать раньше (и purge затем удаляет файл), либо после commit видит объект отсутствующим/удалённым и не возвращает plaintext. Небезопасный путь или ошибка останавливает операцию до удаления KO из БД. Это targeted cleanup конкретного KO, а не массовая очистка legacy-vault.

Реализация (шаги 4–5): purge доступен только через capability `admin.data.purge` (owner+admin). Admin до destructive шага durable пишет body-free `admin.knowledge.purge_attempted`, а после commit — `admin.knowledge.purge`; если вторую запись подтвердить не удалось, API возвращает явный body-free 503 `purged_audit_unconfirmed`, не маскируя уже совершённое удаление generic 500. Публичные Admin/CLI receipts содержат только SHA-256 reference объекта, aggregate counts и cleanup booleans; title, tenant/raw identifiers, body и filesystem paths наружу не возвращаются. CLI фиксирует каждый подтверждённый элемент сразу, поэтому partial batch не теряет аудит предшествующих commit. После предварительного durable vault cleanup `storage.purge_knowledge_object` в одной транзакции удаляет строки во всех связанных таблицах (versions, entity links, usage, embeddings — и пообъектные, и пассажные `knowledge_chunk_embeddings`, inbox, conflicts, feedback/feedback_state) в FK-порядке, зануляет висячие `superseded_by_id` и удаляет сам объект — его `AFTER DELETE`-триггер убирает запись из FTS. Затем `friday.purge.purge_knowledge` удаляет raw-файл (только если объект осиротел, файл не разделяется по content-hash и путь внутри `files_dir`). Объект должен быть **сначала мягко удалён** (`deleted_at`); `list_purgeable_knowledge`/`FRIDAY_PURGE_RETENTION_DAYS` (по умолчанию 30 дней) задают retention-окно. Интерфейсы: `GET /api/admin/data/purgeable`, `POST /api/admin/knowledge/{id}/purge`, и offline-команда `jericho purge --yes` (берёт эксклюзивный backend-lease). Шаг 3 (проверенный export/backup перед purge) остаётся ручным.
