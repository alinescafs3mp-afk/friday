...
- `new` — существенно меньший, типизированный и model-aware оркестратор;
...
- `canary` — ограниченное включение `new` по разрешённым маршрутам/пользователям.
...
Эти части остаются единственными и общими для `legacy` и `new`; копировать или
...
Legacy сначала вызывает эти блоки как адаптер. Будущий `new` забирает их целиком,
...
```text
FRIDAY_ROUTER_MODE=legacy|shadow|canary|new
```

Режим должен дополнительно поддерживать маршрутный allowlist. Это позволит,
например, сначала включить `new` для small-talk и локального файлового чтения,
...
- неизвестный/невалидный режим fail-closed выбирает `legacy`, а не `new`.
...
2. **Каркас (2–3 дня):** typed contracts, `legacy|shadow|canary|new`, telemetry,
...
До отдельного сигнала владельца:

- не создавать `new` runtime;
- не добавлять переключатель;
- не менять production model profile ради будущей модели;
- продолжать текущие исправления через небольшие переиспользуемые контракты;
- сохранять legacy как единственную выдающую логику;
