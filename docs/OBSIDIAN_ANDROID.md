...
В уже развёрнутой immutable-установке эти значения не дописываются прямо в
канонический env работающего релиза. Оператор сначала принимает schema-35
release с выключенным Organ, затем получает owner-private staged ENV1 через
парные `--next-env-file`/`--next-env-file-sha256`. ENV1 становится каноническим
только после проверенной единой копии SQLite, Telegram inbox и exact vault root;
staged-копия после успешного cutover удаляется. Полный порядок и recovery
граница описаны в [OPERATIONS.md](OPERATIONS.md#obsidian-android-beta).
...
## Команды в обычном Telegram-диалоге

Сначала `/obsidian` должен показывать onboarding `ready`. Затем отправляйте в
личный чат одну операцию за сообщение; пути задаются относительно корня vault и
должны оканчиваться на `.md`. Точные поддерживаемые шаблоны первого релиза:

```text
Покажи хранилища Obsidian.
...
Friday выполняет ровно одну выбранную операцию и не считает ответ модели
доказательством успеха. В квитанции отдельно показаны локальная запись,
server scan, текущее подключение Android и получение точной revision. Если
квитанция неполна или результат `uncertain`, автоматического повтора нет:
сначала проверьте vault и `/obsidian`.
