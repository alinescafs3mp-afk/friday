...
- **Охват не растёт** (68.8% в обоих случаях). Он упирается в состав пула, а не в порядок: там, где отвечающих документов нет вовсе, переставлять нечего. Переранжирование не расширяет поиск и не может этого делать по построению.
