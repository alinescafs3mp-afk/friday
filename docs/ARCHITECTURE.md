...
Он **намеренно не подключён** к `HybridSearcher` и к фоновому контексту агента:
исходники не подмешиваются в каждый разговор. Единственная агентская поверхность —
явный capability-gated инструмент `source_search`, когда человек просит найти
сведения именно в загруженном/присланном исходнике. Он возвращает только
query-aware выдержки, помечает `pending` как материал на review (не как
долгосрочное знание) и сообщает, когда порция не доказывает полноту. Это позволяет
искать в только что принятом файле до promotion, не превращая весь Raw-корпус в
шестой фоновый канал recall.
...
Код задаёт модели доступный контекст, инструменты и проверяемые границы, но не
подменяет её понимание языка исчерпывающими regex и эвристиками. Такие
классификаторы допустимы как измеренный быстрый маршрут или положительная
подсказка, но не как отрицательный гейт, объявляющий разрешённую задачу
невозможной. Детерминированный fail-closed обязателен на реальных границах:
authorization/tenant/privacy, опасные side effects, provenance/evidence и
completeness, ресурсные лимиты и фактическая недоступность способности. Внутри
разрешённой области модель решает задачу по полному релевантному контексту.
...
Режимы определяют глубину работы, а не подменяют личность актора. `dialogue`
минимизирует tool use, `knowledge_work` допускает несколько шагов над личным
контекстом, `research` получает расширенный bounded budget и обязан отделять
план, evidence и synthesis. `engineer` — выключенный по умолчанию автономный
верстак владельца установки. В его боевом контуре аутентифицированная текущая
реплика из личного Telegram-чата делегируется primary-модели: она сама планирует
цепочку и вызывает произвольный Bash от непривилегированного пользователя
службы с его реальными PATH, файловой системой и сетью. Командные/path/network
allowlist и `/approvals` в этом контуре отсутствуют; границей остаются точная
личность владельца и реальные права ОС. Текущие вложения входят как
повторно авторизованный immutable input batch, рабочее состояние сохраняется в
conversation-scoped workbench, а готовые файлы публикуются через отдельный
output-каталог и durable Telegram fence. Старые bounded Engineer tools не
предлагаются модели при включённом универсальном runner, но остаются
совместимым fallback при его отключении. Нормативный контракт:
