...
- **Охват не растёт** (68.8% в обоих случаях). Он упирается в состав пула, а не в порядок: там, где отвечающих документов нет вовсе, переставлять нечего. Переранжирование не расширяет поиск и не может этого делать по построению — оно только меняет порядок, состав остаётся прежним, и проверка длины это стережёт.
...
- **Отказ службы не роняет поиск** — выдача остаётся в прежнем порядке. Именно поэтому отказ проверяется диагностикой (`start_rerank_runtime`): молчаливый откат к AUC 0.512 не меняет ни одного признака, и заметить его по виду выдачи нельзя.
