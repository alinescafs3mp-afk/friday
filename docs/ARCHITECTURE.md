...
`_prepare_context` теперь вычисляет `graph_expansion` через единый `is_relational_query`: обычный запрос получает `False`, а распознанный запрос об отношениях — `True`. `/api/search` и явный `memory_search` сохраняют свой публичный контракт; default параметра поиска не менялся.
