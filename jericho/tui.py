"""Pseudographic launcher for Jericho (``jericho tui``).
...
Jericho at a model on the LAN does not require hand-editing env files.
...
LLM_FIELDS: tuple[tuple[str, str, bool], ...] = (
    ("JERICHO_LLM_BASE_URL", "LLM base URL", False),
    ("JERICHO_LLM_MODEL", "LLM model", False),
    ("JERICHO_LLM_ENABLED", "LLM enabled (1/0)", False),
    ("JERICHO_LLM_API_KEY", "LLM API token", True),
    ("JERICHO_EMBEDDINGS_BASE_URL", "Embeddings base URL", False),
    ("JERICHO_EMBEDDINGS_API_KEY", "Embeddings token", True),
)
...
def apply_values(text: str, values: dict[str, str]) -> str:
    ...
    fallback still fires (e.g. ``JERICHO_EMBEDDINGS_API_KEY`` → the LLM key). Writing
...
def env_path(env_file: str | None = None) -> Path:
    """Resolve the target ``.env.local`` (explicit → JERICHO_ENV_FILE → cwd)."""

    candidate = env_file or os.environ.get("JERICHO_ENV_FILE") or str(Path.cwd() / ".env.local")
...
def _gather_status(settings, values: dict[str, str]) -> str:
    ...
    effective = replace(
        settings,
        llm_base_url=(values.get("JERICHO_LLM_BASE_URL") or settings.llm_base_url).rstrip("/"),
        llm_model=values.get("JERICHO_LLM_MODEL") or settings.llm_model,
        llm_api_key=values.get("JERICHO_LLM_API_KEY", settings.llm_api_key),
    )
    try:
...
def _save(state: LauncherState, target: Path) -> None:
    from jericho.cli import _write_private_text_atomic
...
def _render(stdscr, state: LauncherState) -> None:
    ...
    with contextlib.suppress(curses.error):
        stdscr.border()
    _safe(stdscr, 0, 2, " Jericho — лаунчер ", curses.A_BOLD)
    if state.screen == "main":
...
def _exec_up(env_file: str | None) -> None:
    ...
    argv = [sys.executable, "-m", "jericho.cli"]
    if env_file:
