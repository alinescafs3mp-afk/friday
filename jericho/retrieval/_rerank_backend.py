...
# архиве «график дежурств» даёт пять штук по 0.999, а вопрос, ответа на который в
...
#
# Порог почти удваивает долю отвечающих и заставляет молчать там, где ответа нет, но
# стоит 16% вопросов, у которых ответ был. Поэтому он НЕ режет выдачу: число «сколько
# из показанного похоже на ответ» уходит в `strategy`, а решение отсекать — к
# человеку. Тот же принцип, что у порога плотных доказательств: значение принадлежит
# МОДЕЛИ, у другой шкала будет другой, поэтому оно настраиваемое.
